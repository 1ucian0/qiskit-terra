...
class TestQuantumCircuitFindBit(QiskitTestCase):
    ...
    def test_bit_not_in_circuit(self, bit_type):
        ...
        with self.assertRaisesRegex(CircuitError, r""):
            qc.find_bit(bit)

    @data(Qubit, Clbit)
    def test_registerless_bit_constr(self, bit_type):
    ...
    def test_registerless_add_bits(self, bit_type):
        """Verify we find individual bits added via QuantumCicrcuit.add_bits."""
        bits = [bit_type() for _ in range(5)]
    ...
    def test_registerless_add_int(self):
        """Verify we find individual bits added via QuantumCicrcuit(int, int)."""
    ...
    def test_register_bit_add_reg(self, reg_type):
        """Verify we find register bits added QuantumCicrcuit.add_register."""
        reg = reg_type(5, "reg")
